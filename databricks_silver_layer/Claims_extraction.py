...
# Add the 'datasource' column based on file name
claims_df = claims_df.withColumn(
    "datasource",
    f.when(f.input_file_name().like("%hospital1%"), "hosa")
     .when(f.input_file_name().like("%hospital2%"), "hosb")
     .otherwise("unknown")  # Debugging step
)
...
hosa_count = claims_df.filter(col("datasource") == "hosa").count()
print(f"Total records where datasource = 'hosa': {hosa_count}")
...
# MAGIC %sql
# MAGIC CREATE TABLE IF NOT EXISTS healthcare_insurance.silver.claims (
...
# MAGIC )
# MAGIC USING DELTA LOCATION 'abfss://silver@healthinsurance.dfs.core.windows.net/claims';

# COMMAND ----------

# MAGIC %sql
# MAGIC -- Update old record to implement SCD Type 2
# MAGIC MERGE INTO healthcare_insurance.silver.claims AS target
...
# MAGIC %sql
# MAGIC select * from  healthcare_insurance.silver.claims
# MAGIC
